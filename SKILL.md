...
「假單簽核」/「異常簽核」四個清單，依下方標準政策核准待簽項目，然後回報
結果。
...
- **夜點津貼簽核**：完全跳過——不檢查也不回報。
- **異常簽核**：預設每次都檢查、回報，不核准——沒有白名單、也沒有單次
  授權機制。唯一會核准的情況是明確的 `APPROVE_ALL=1` 覆寫（見下方），跟
  加班單/假單在那個模式下一樣是全有或全無，平常檢查不會自己觸發。
...
- **每次執行都會寫入本機記錄**：`check-queues.js` 每次執行都會把每一筆待簽
  項目（外加空佇列的摘要列）追加寫入
  `playwright/logs/approval-log-YYYY-MM.csv`——欄位有 timestamp、queue、
  status（empty/approved/still_pending/reported）、action、name、id、
  detail。每一列是依照**事件本身的日期**歸檔（例如 8/29 的加班單就算是
  9 月才被檢查到，還是會存進 8 月的檔案），只有「空佇列」摘要列沒有事件
  日期可用，歸入執行當月的檔案。這純粹是本機稽核用途（已加入
  .gitignore），不需要主動跟使用者報告，除非對方問起。
...
1. **執行檢查**：
   ...
   ```
   這一個腳本會做完所有事：登入已存的 session、依序造訪四個佇列、透過
   DOM 讀取待簽項目（不用截圖），並且**在回傳結果前就已經執行完**標準
   政策的核准動作（預定加班單簽核全部自動核准、加班單簽核核准白名單子集）
   ——這不是預演/dry run。異常簽核在一般執行下僅供查看（要核准需要明確的
   `APPROVE_ALL=1` 覆寫——見上方「標準政策」）。會先印出人類可讀的過程紀錄，
   最後印出一行 `RESULT_JSON:{...}`。要解析這最後一行 JSON 拿結構化結果；
   前面的過程紀錄只是輔助除錯用，不用照著解析。
...
3. **依標準政策回報給使用者**，內容取自 JSON：哪個佇列有幾筆待簽、哪些
   ...
   （假單簽核的項目、不在白名單上的加班單簽核項目、以及異常簽核項目——
   這一項只有在明確跑過 `APPROVE_ALL=1` 才會被核准，一般檢查不會），以及
   目前時間。
...
## 跨夜保活（從源頭避免 session 過期）

依實際觀察，差勤系統伺服器端的閒置逾時時間，長到能撐過白天檢查排程之間
較短的間隔，但撐不過較長的跨夜空窗（例如晚上最後一次檢查到隔天早上第一
次檢查之間）——那段空窗會穩定地讓 session 死掉。`playwright/keep-alive.js`
就是為了解決這個問題：單純訪問 `mainback.asp`，不做任何其他事——不讀取
佇列、不核准、不寫入 CSV 記錄——只是為了重置閒置計時器。會印出 `ALIVE`
或 `EXPIRED`，不管哪種都會以 exit code 0 結束（這是一個狀態探測，不是
一個「會失敗」的任務）。透過 CronCreate 排程在跨夜空窗期間多打幾次
（實際時間點要依你自己系統的閒置逾時長度調整）。如果回報 `EXPIRED`，
半夜不用主動處理，記錄下來、等下一次正式檢查（有實際的 session 重新整理
邏輯）在使用者清醒的時段處理即可。
