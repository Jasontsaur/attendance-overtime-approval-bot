...
- **夜點津貼簽核**：完全跳過——不檢查也不回報。
- **異常簽核**：每次都檢查、回報，但這個 skill 完全不核准這個佇列裡的任何
  東西——沒有白名單、也沒有單次授權機制，就只是把待簽項目列出來而已。
...
1. **執行檢查**：
   ...
   ——這不是預演/dry run。異常簽核是唯讀的（完全沒有核准邏輯）。會先印出
   人類可讀的過程紀錄，最後印出一行 `RESULT_JSON:{...}`。要解析這最後一行
   JSON 拿結構化結果；前面的過程紀錄只是輔助除錯用，不用照著解析。
